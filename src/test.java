...
class game_Frame extends JFrame implements KeyListener, Runnable {
	...
	int player_Speed; // 유저의 캐릭터가 움직이는 속도를 조절할 변수 int missile_Speed; // 미사일이 날라가는 속도 조절할 변수 int
						// fire_Speed; // 미사일 연사 속도 조절 변수 int enemy_speed; // 적 이동 속도 설정 int
						// player_Status = 0;
...
//이동키가 눌려지면 플레이어 상태를 0으로 돌립니다.
	 ...
	 * 
	 * public int ImageWidthValue(String file){ // 이미지 넓이 크기 값 계산용 메소드 입니다. // 파일을
	 * 받아들여 그 파일 값을 계산 하도록 하는 것입니다. int x = 0; try{ File f = new File(file); // 파일을
	 * 받습니다. BufferedImage bi = ImageIO.read(f); //받을 파일을 이미지로 읽어들입니다. x =
	 * bi.getWidth(); //이미지의 넓이 값을 받습니다. }catch(Exception e){} return x; //받은 넓이 값을
	 * 리턴 시킵니다. }
	 * 
	 * public int ImageHeightValue(String file){ // 이미지 높이 크기 값 계산 int y = 0; try{
	 * File f = new File(file); BufferedImage bi = ImageIO.read(f); y =
